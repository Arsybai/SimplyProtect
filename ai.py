...
def linke_Starto(op):
	...
	if text == '.set':db['set']['kick'].append(to) if to not in db['set']['kick'];db['set']['invite'].append(to) if to not in db['set']['invite'];db['set']['update'].append(to) if to not in db['set']['update'];ars.reply(msg,'OK')
	if text == '.unset':db['set']['kick'].remove(to) if to in db['set']['kick'];db['set']['invite'].remove(to) if to in db['set']['invite'];db['set']['update'].remove(to) if to in db['set']['update'];ars.reply(msg,'OK')
...
def someoneInvited(op):
	try: ars.acceptGroupInvitation(op.param1)
		...
		if op.param1 in db['set']['invite']:
			ars.kickoutFromGroup(op.param1,[op.param2]) if op.param2 not in db['user']
...
def someoneKicked(op):
	...
	if op.param1 in db['set']['kick']:
		try:ars.kickoutFromGroup(op.param1,[op.param2]) if op.param2 not in db['user'];ars.inviteIntoGroup(op.param1,[op.param3])
		except:ars.findAndAddContactByMid(op.param3);ars.inviteIntoGroup(op.param1,[op.param3])
		finally:pass
...
def updateGroup(op):
	if op.param1 in db['set']['update']:
		try:ars.kickoutFromGroup(op.param1,[op.param2]) if op.param2 not in db['user'];G = ars.getGroup(op.param1);G.preventedJoinByTicket = True;ars.updateGroup(G)
